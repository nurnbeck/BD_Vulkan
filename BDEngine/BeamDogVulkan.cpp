...
namespace BDEngine
{
	...
	void BeamDog::InitVulkan()
	{
		...
		graphicsCommandPool = new BDCommandPool(&logicalDevice, indices.graphicsFamily);
		transferCommandPool = new BDCommandPool(&logicalDevice, indices.transferFamily , VK_COMMAND_POOL_CREATE_TRANSIENT_BIT);

		vertexBuffer = new BDVertexBuffer(&logicalDevice, &physicalDevice, &surface, transferCommandPool->GetCommandPool(), &transferQueue);
		indexBuffer = new BDIndexBuffer(&logicalDevice, &physicalDevice, &surface, transferCommandPool->GetCommandPool(), &transferQueue);
		uniformBuffer = new BDUniformBuffer(&logicalDevice, &physicalDevice, &surface, transferCommandPool->GetCommandPool(), &transferQueue);
	...
	void BeamDog::InitWindow()
	{
		windowObj.Create();
		glfwSetWindowUserPointer(windowObj.window, this);
		glfwSetWindowSizeCallback(windowObj.window,BeamDog::OnWindowResized);
	}

	void BeamDog::RecreateSwapChain()
	{
		vkDeviceWaitIdle(logicalDevice);
		CleanupSwapChain();
	...
	bool BeamDog::CheckValidationLayerSupport()
	{
	...
	std::vector<const char*> BeamDog::GetRequiredExtensions()
	{
	...
	void BeamDog::SetupDebugCallback()
	{
		if (!enableValidationLayers)
		{
			return;
	...
	void BeamDog::GetPhysicalDevices()
	{
	...
	SwapChainSupportDetails BeamDog::QuerySwapChainSupport(VkPhysicalDevice device)
	{
	...
	int BeamDog::RateDeviceSuitability(VkPhysicalDevice deviceToRate)
	{
		int score = 0;
	...
	void BeamDog::CreateLogicalDevice()
	{
		QueueFamilyIndices indices = FindQueueFamilies(&physicalDevice, &surface);
	...
	void BeamDog::CreateDescriptorSetlayout()
	{
	...
	void BeamDog::CreateDescriptorPool()
	{
	...
	void BeamDog::CreateDescriptorSet()
	{
	...
	VkShaderModule BeamDog::CreateShaderModule(const std::vector<char>& code)
	{
	...
	void BeamDog::CreateCommandBuffers()
	{
	...
	bool BeamDog::CheckDeviceExtensionSupport(VkPhysicalDevice device)
	{
